export const V2RedirectRoutes: {[orginal: string]: string} = {
  article_publish_time_viewer: "/article-publish-time-viewer",
  article_wordcloud_generator: "/article-wordcloud-generator",
  URL_scheme_convertor: "/URL-scheme-convertor",
  lottery_data_analyze: "/lottery-analyzer",
  lottery_reward_record_viewer: "/lottery-reward-record-viewer",
  JPEP_FTN_macket_analyzer: "/JPEP-FTN-market-analyzer",
  LP_recommend_checker: "/LP-recommend-checker",
  on_rank_article_viewer: "/on-rank-article-viewer",
  user_VIP_status_viewer: "/VIP-info-viewer",
};

export const V2UnavaliableRoutes: string[] = ["diszeroer_helper"];

export const V2UnimplementedRoutes: string[] = ["article_downloader", "thanks"];
